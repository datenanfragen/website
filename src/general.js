...
Object.defineProperty(globals, 'country', {
    set: function(country) {
        ...
        this._country_listeners.forEach(function(listener) { listener(country); });
    },
    get: function() { return Cookie.get('country'); },
});

if(!globals.country) globals.country = guessUserCountry();

document.querySelectorAll('.i18n-button-container').forEach(el => { preact.render(<I18nButton />, el); });
...
if(Privacy.isAllowed(PRIVACY_ACTIONS.SAVE_ID_DATA)) {
    preact.render(
        ...
        </div>, document.getElementById('id-data-controls'));
}
...
function guessUserCountry() {
    ...
    let navigator_lang = (navigator.languages && navigator.languages.length) ? navigator.languages[0] : navigator.language || navigator.browserLanguage;
